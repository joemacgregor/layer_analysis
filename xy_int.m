% XY_INT Determine intersections of radar transects.
% 
%   XY_INT determines the positions of intersections between each
...
if do_xy
    ...
    name_year               = name_year(~strcmp(name_year, 'test'));
    name_year               = name_year(~strcmp(name_year, 'fig'));
    name_year               = name_year(~strcmp(name_year, 'AGAP'));
    name_year               = name_year(~strcmp(name_year, '2011_accum'));
    name_year               = name_year(~strcmp(name_year, '2012_accum'));
    name_year               = name_year(~strcmp(name_year, '2013_accum'));
        ...
        for jj = 1:num_trans(ii)
            ...
            for kk = 1:num_curr
                ...
                if (ind_tmp < num_decim)
                    ...
                    [dist_close, ind_close]         = min((sqrt(((x{ii}{jj}(decim_vec((ind_tmp + 1):num_decim)) - x{ii}{jj}(kk)) .^ 2) + ...
                                                                ((y{ii}{jj}(decim_vec((ind_tmp + 1):num_decim)) - y{ii}{jj}(kk)) .^ 2))));
            ...
            if (ii ~= jj) % transects from different years
                ...
                for kk = 1:num_trans(ii)
                    ...
                    for ll = 1:num_trans(jj)
                        ...
                        [~, ~, ind_int1, ind_int2]      = intersecti(x{ii}{kk}(ind_tmp1), y{ii}{kk}(ind_tmp1), x{jj}{ll}(ind_tmp2), y{jj}{ll}(ind_tmp2));
