function [age, age_ord, age_n, age_range, age_type, age_uncert, date_counter] ...
                            = date_interp_shallow(depth, thick, age, age_ord, age_n, age_range, age_type, age_uncert, interp_type, date_counter, curr_layer, age_uncert_rel_max, depth_shallow_min, depth_shallow_max, age_max, do_age_check, curr_name, age_type_assign)
...
[age_bound_tmp, age_uncert_slice_tmp, depth_bound_tmp, depth_curr, ind_overlap_bounded, depth_min, depth_max, thick_curr] ...
                            = deal(age(~isnan(age)), age_uncert(~isnan(age_uncert)), depth(~isnan(age), ind_overlap), depth(curr_layer, ind_overlap), false(1, num_overlap_max), (depth_shallow_min * thick(ind_overlap)), (depth_shallow_max * thick(ind_overlap)), thick(ind_overlap));
...
age_overlap_uncert          = sqrt(nanvar(age_overlap) + nanmean((age_uncert_bound .^ 2), 2));
